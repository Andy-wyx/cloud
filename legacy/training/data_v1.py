...
def preprocess_txt(text):
    res = tokenize([str(text)])
    #res = word_tokenize(str(text))
    #print(res[0])  # 77
    #print(res.shape) # 1 x 77
    #import time 
    #time.sleep(100)
    return res
...
# def get_coco_dataset(args, preprocess_img, is_train, epoch=0):
#     input_shards = args.train_data if is_train else args.val_data
#     assert input_shards is not None

#     num_samples, num_shards = get_dataset_size(input_shards)
#     if not num_samples:
#         raise RuntimeError('Number of dataset samples must be specified.')
...
#         pipeline.extend([
#             wds.split_by_worker,
#             lambda x: coco_loader(x, args.anno_path),  # 加载 COCO 数据和标注
#         ])
#     else:
#         pipeline.extend([
#             wds.split_by_worker,
#             lambda x: coco_loader(x, args.anno_path),
#         ])

#     pipeline.extend([
#         wds.map_dict(image=preprocess_img, bbox=preprocess_bbox),  # 假设你需要处理图像和边界框
#         wds.to_tuple("image", "bbox"),
#         wds.batched(args.batch_size, partial=not is_train),
#     ])
...
# def coco_loader(data_path, anno_path, is_train=True):
#     # 初始化 COCO api
#     coco = COCO(anno_path)
#     # 获取图像 id 列表
